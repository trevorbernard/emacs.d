...
performance. We'll reset it back to default at the end of the file.
...
#+begin_src elisp
  ...
  (use-package ivy
    :ensure t
    :config
    (ivy-mode 1))  ; Ivy is my preferred completion framework

  ;; LSP + Ivy integration for symbol search
  (use-package lsp-ivy
    :ensure t
    :commands lsp-ivy-workspace-symbol)  ; Search for symbols across the workspace
  ...
  (use-package lsp-ui
    ...
    :config
    (setq lsp-ui-doc-enable nil))  ; I prefer to trigger docs manually

  ;; The main LSP mode package
  (use-package lsp-mode
    :ensure t
    :commands (lsp lsp-deferred)  ; Only load when needed
    :hook
    ;; Automatically start LSP when opening these file types
