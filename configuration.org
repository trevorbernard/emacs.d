...
- [[#terminal--clipboard-integration][Terminal & Clipboard Integration]]
- [[#key-bindings][Key Bindings]]
- [[#editor-behavior--preferences][Editor Behavior & Preferences]]
...
Visual configuration for a clean, modern Emacs experience with consistent
theming and helpful visual indicators.
...
#+begin_src emacs-lisp
  (use-package doom-modeline
    ...
    :hook (after-init . (lambda () (run-with-idle-timer 0 nil #'doom-modeline-mode))))
#+end_src
...
Different operating systems and display environments require specific
configurations for optimal Emacs integration.
...
Modern Emacs needs seamless integration between terminal and GUI environments,
especially when working over SSH or in mixed Wayland/X11 setups. This section
consolidates all terminal-specific configurations and clipboard handling.
...
*** Terminal Mouse & Display Support
...
# vterm provides a fully-fledged terminal emulator that handles escape
# sequences properly, making it superior to the built-in term/ansi-term.
...
When working in a terminal over SSH, OSC-52 allows yanked text in Emacs
to become available on the host machine's clipboard.
...
Global key bindings and keyboard shortcuts for improved ergonomics.
...
#+begin_src emacs-lisp
  ;; Cider REPL jack-in - long-time habit
  (keymap-global-set "C-c C-j" 'cider-jack-in)

  ;; Magit bindings for git operations
...
VIM motions are clearly the superior way to edit text but I'm hardcoded to Emacs
text editting. This will be a long transition.
...
Core Emacs settings that affect how the editor behaves and feels during
everyday use.
...
#+begin_src emacs-lisp
  ...
  ;; Allow delete of selection when typing
...
*** File Handling & Backup Preferences

By default, Emacs generates backup files, auto-save files, and lockfiles.
With modern system stability and version control, these are often redundant.
...
#+begin_src emacs-lisp
  ;; Allow these functions without confirmation warnings
...
Programming language-specific configurations and integrations.
...
#+begin_src emacs-lisp
  (use-package ivy
    :ensure t
    :commands ivy-mode
    :hook (after-init . (lambda () (run-with-idle-timer 0 nil #'ivy-mode))))
...
#+begin_src emacs-lisp
  ...
  (use-package flymake-pest :ensure t)
  (use-package flycheck-pest :ensure t)
#+end_src
